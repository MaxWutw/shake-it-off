...
Tray Realized as a Real-Time Cyber-Physical System}
...
\begin{abstract}
We present \emph{The Good Plate}, a self-stabilizing adaptive tray that keeps
...
Using on-chip cycle-accurate timing across $36{,}031$ control cycles we show the
task set is provably schedulable ($\sum_i C_i = 1629\,\mu\text{s} \le D =
2000\,\mu\text{s}$) with \mbox{0.00\%} deadline misses. Because our controller
omits an explicit rate-damping term, the Lyapunov derivative cannot be signed
analytically; we therefore certify stability \emph{quantitatively from logged
...
perform by continuously adjusting the tray angle to keep it level. We set out
...
Rather than build a fully enclosed tray, we built the underlying
\emph{self-stabilizing platform}, which is the hard part. The platform rests on
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=0.78, every node/.style={font=\scriptsize}]
    % Base
    \draw[fill=orange!20, draw=orange!80!black, thick] (-4,-2) rectangle (4,-1.7);
    \node[text=orange!60!black] at (0,-1.85) {\textbf{Base}};
    % Lower pillar + joint
    \draw[fill=gray!30, draw=black, thick] (-0.15,-1.7) rectangle (0.15,0);
    \draw[fill=gray!50, draw=black] (0,0) circle (0.15);
    \node[anchor=west] at (0.2,-0.6) {Universal joint};
    % Platform (tilted)
    ...
    % Servos
    \draw[fill=red!30, draw=red!70!black, thick] (-2.5,-1.7) rectangle (-2,-1.0);
    \draw[fill=red!30, draw=red!70!black, thick] (2.0,-1.7) rectangle (2.5,-1.0);
    ...
    \node[rotate=10, text=green!50!black] at (-1.2,0.05) {\tiny IMU};
  \end{tikzpicture}
...
\underbrace{\text{PI control}}_{\tau_3}\!\rightarrow\!
...
integrated angle drifts over time. We fuse them with a fixed-weight
...
online, but the fixed-weight form is sufficient for our low-speed platform and
costs only one multiply and one add per axis (Table~\ref{tab:tasks}). We
deliberately keep $\alpha$ high to favor the gyro short-term and let the
...
$e(t) = \theta_\text{ref}-\theta(t) = -\theta(t)$. We want a control law that
...
noise on a microcontroller. We therefore drop $K_d$ and implement a
\textbf{velocity-form PI}:
\begin{equation}
\boxed{\;\Delta u_k = K_{P}\,[e_k-e_{k-1}] + K_{I}\, e_k\;}
\label{eq:velpi}
\end{equation}
with $K_{P}=0.1$, $K_{I}=0.01$ in our firmware. The controller output is the
...
strings connect a rotating platform to fixed servo positions. We instead compute
the \emph{exact} string length required at the desired tilt and convert that
length to a servo rotation. The geometry (per axis) is shown in
...
\begin{figure}[t]
  ...
  \begin{picture}(220,150)
    % base line
    \put(0,20){\line(1,0){220}}
    % servos
    \put(8,12){\framebox(18,10){}}
    \put(194,12){\framebox(18,10){}}
    \put(2,2){\scriptsize $(-k,0)$}
    \put(188,2){\scriptsize $(k,0)$}
    \put(104,2){\scriptsize $(0,0)$}
    % lower pillar b
    \put(110,20){\line(0,1){55}}
    \put(114,46){\scriptsize $b$}
    % joint
    \put(110,75){\circle*{6}}
    \put(116,72){\scriptsize joint $(0,b)$}
    % upper pillar a (tilted)
    \put(110,75){\line(-1,4){10}}
    \put(86,96){\scriptsize $a$}
    % platform
    \put(18,52){\line(3,1){182}}
    \put(18,52){\circle*{4}}
    \put(200,112){\circle*{4}}
    \put(0,44){\scriptsize $P_L$}
    \put(196,118){\scriptsize $P_R$}
    % strings
    \put(18,52){\line(0,-1){32}}
    \put(200,112){\line(0,-1){92}}
    \put(6,34){\scriptsize $\ell_L$}
    \put(204,70){\scriptsize $\ell_R$}
    % theta arc
    \put(100,115){\scriptsize $\theta$}
  \end{picture}
  \end{minipage}
...
\end{equation}
Keeping the $a$ term is what distinguishes our model from a naive single-pillar
...
\end{equation}
so both strings have the neutral length $a+b$ when level, as required. We work
with the length \emph{change} relative to neutral,
...
in our build, the $\Delta\ell_R=+1.96$\,cm of the example maps to a servo
...
(500\,Hz). We instrumented each task with the Cortex-M4 Data Watchpoint and
Trace (DWT) cycle counter~\cite{arm2010cortexm4} (at 84\,MHz, $1\,\mu\text{s} =
84$ cycles) and logged $36{,}031$ cycles. The per-task statistics are listed in
...
Our measured $U=81.5\%$ \emph{exceeds} this bound, but the bound does not
apply: it is a \emph{sufficient} test for \emph{preemptive} RM scheduling of
independent periodic tasks, whereas our tasks are a single non-preemptive
...
Take the state $\mathbf x=[\theta_p,\theta_r]^\top$ with equilibrium
...
$\mathbf x\ne\mathbf 0$~\cite{khalil2002nonlinear,slotine1991applied}. We cannot
establish this on paper for two reasons. First, our controller is
...
guaranteed $-c\,\omega^2$ into \eqref{eq:vdot}. Second, we have no identified
...
strictly positive during every overshoot of an under-damped response. We
therefore do \emph{not} claim asymptotic stability analytically; instead we
verify the decrease condition \emph{quantitatively from logged data} using two
...
$\dot V\approx (V[k{+}1]-V[k])/\Delta t$ with $\Delta t>0$, we have
$\operatorname{sign}(\Delta V)=\operatorname{sign}(\dot V)$, so $\rho_{\Delta V}$
...
Our data give $\rho_{\Delta V}=49.7\%$ on average. This is \emph{not} a sign of
...
even as its peaks shrink. A per-sample count near $50\%$ therefore tells us the
...
$\ln V(t)=\ln V_0-\gamma t$. We therefore extract $\gamma$ per disturbance event
...
system has $\rho_{\Delta V}\approx 50\%$ \emph{and} $\gamma>0$---precisely our
case.

\subsection{Results}
We applied a quantitative analysis to the $80\,\text{s}$, $36{,}031$-sample log,
...
% =====================================================
We built and verified a complete cyber-physical control loop---IMU $\rightarrow$
complementary filter $\rightarrow$ velocity-form PI $\rightarrow$ exact
string-length inverse kinematics $\rightarrow$ PWM---for a self-stabilizing
...
identified plant model, we certified stability quantitatively from data: the
