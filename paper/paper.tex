...
\begin{IEEEkeywords}
  real-time systems, self-stabilization, complementary filter, PI control,
...
% =====================================================
Carrying a tray of objects such as cups, plates, and instruments is a task humans
...
\begin{enumerate}
  \item A complete embedded real-time control pipeline (IMU $\rightarrow$ complementary
...
the \emph{length of a string}. A naive mapping that increments the servo angle by
...
\mbox{$a=2.2$}, $b=4.5$, $k=11.4$ (cm) and a mid-range $\theta=5^\circ$,
\begin{align*}
  \ell_R(5^\circ) & \approx 7.69\,\text{cm}\;(\Delta\ell_R\approx+0.99\,\text{cm, pay out}), \\
  \ell_L(5^\circ) & \approx 5.70\,\text{cm}\;(\Delta\ell_L\approx-1.00\,\text{cm, reel in}).
\end{align*}
...
in this implementation, the $\Delta\ell_R=+0.99$\,cm of the example maps to a servo
rotation of $\alpha\approx70^\circ$, i.e.\ a PWM command of
$\theta_\text{servo}\approx160^\circ$, well within the $0$--$180^\circ$ range.
Since $\alpha=360^\circ\,\Delta\ell/C$, a
\emph{larger} spool circumference winds more string per revolution and therefore
extends the achievable length range at the cost of angular resolution, whereas a
\emph{smaller} circumference gives finer resolution but less range.
...
\begin{figure*}[t]
  ...
  \caption{Timing diagram over seven consecutive control periods. The IMU read
    $\tau_1$ ($\sim$1558\,$\mu$s) dominates; $\tau_2$--$\tau_5$ are negligible by
...
\end{equation}
leaving a slack of $D-\sum_i C_i = 393\,\mu\text{s}$ (a 19.7\% margin). Because
...
(Section~\ref{sec:control}), so there is \emph{no explicit rate-damping} that would inject a
...
log, in which the base was tilted four times by up to $\sim$$3^\circ$
    ...
    $\tau\!\approx\!19$--$20\,$s) versus a brief tap (event \#3, $\tau\!\approx\!0.2\,$s).
      ...
      of radius $\sim$$1.1^\circ$. Combining the three facts that (i) $V$ is bounded,
    ...
    % =====================================================
    A complete real-time control loop (IMU $\rightarrow$
