...
\graphicspath{{../analysis_output/}{img/}}
...
\begin{abstract}
  ...
  task set is shown to be provably schedulable ($\sum_i C_i = 1607\,\mu\text{s} \le D =
  ...
  data show a bounded energy that settles into a $\sim\!1.1^\circ$ residual ball,
  establishing \emph{practical (ultimately bounded) stability}.
\end{abstract}
...
\end{equation}
where the gravity vector $(0,0,1)^\top$ is subtracted from the accelerometer average. Corrected physical values are computed dynamically in the control loop as:
\begin{equation}
  a_\text{corrected} = \frac{a_\text{raw}}{16384} - a_\text{off},\quad
  \omega_\text{corrected} = \frac{\omega_\text{raw}}{131} - \omega_\text{off}.
\end{equation}

Under a standard theoretical convention where roll $\theta_r$ rotates about body $x$ and pitch $\theta_p$ rotates about body $y$, passive rotation of gravity $\vec g_W = (0,0,-g)^\top$ into the body frame via $R(\theta_p, \theta_r) = R_x(\theta_r) R_y(\theta_p)$ yields the body accelerometer specific force components:
\begin{equation}
  a_x = \sin\theta_p,\quad a_y = -\sin\theta_r\cos\theta_p,\quad a_z = \cos\theta_r\cos\theta_p.
\end{equation}
Isolating the terms yields the standard theoretical tilt equations:
\begin{equation}
  \theta_p^\text{acc} = \operatorname{atan2}\big(a_x,\,\sqrt{a_y^2+a_z^2}\big),\quad
  \theta_r^\text{acc} = \operatorname{atan2}\big(-a_y,\,a_z\big).
\end{equation}
However, on the physical prototype, the IMU is mounted rotated by $90^\circ$ around the yaw axis, effectively swapping the body $x$ and $y$ sensor axes. Hence, the actual firmware implements the coordinate-mapped version:
\begin{equation}
...
The integral action is theoretically required to eliminate steady-state error. Applying the Laplace-domain Final Value Theorem, the steady-state error $e_{ss}$ for the closed-loop system under a step disturbance $T_d(s) = T_0/s$ is:
\begin{equation}
  e_{ss} = \lim_{t\to\infty} e(t) = \lim_{s\to 0} s\,E(s) = \lim_{s\to 0} \frac{s \cdot T_0/s}{1 + G_p(s)C(s)},
\end{equation}
where $G_p(s)$ is the plant and $C(s)$ is the controller. Since the PI controller transfer function $C(s) = K_p + K_i/s$ contains an integrator pole at the origin, we have $\lim_{s\to 0} C(s) = \infty$, which mathematically guarantees $e_{ss} = 0$.
...
\begin{table}[t]
  \caption{Measured Task Execution Times ($N=36{,}122$ cycles)}
  \label{tab:tasks}
  \centering
  \small
...
\subsection{Results}
A quantitative analysis was applied to the $80\,\text{s}$, $36{,}122$-sample log,
which contains two disturbance events (Fig.~\ref{fig:lyap}). The per-event
results are in Table~\ref{tab:lyap}. The energy is bounded throughout
($V_{\max}=4.93<\infty$, no escape). Because the events are long and contain steady-state noise, the fitted decay rates are near zero or slightly negative ($\gamma = -0.070\,\text{s}^{-1}$ on average), but the trajectory remains bounded. The $\dot V\le0$ fraction sits at $50.5\%$, confirming the under-damped-but-bounded picture.
...
\begin{table}[t]
  ...
  \begin{tabular}{ccccc}
    ...
    \#2          & 4.93            & -0.140              & ---          & 50.7\%            \\
    \midrule
    \textbf{avg} & ---             & \textbf{-0.070}     & \textbf{---} & \textbf{50.5\%}   \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Empirical Lyapunov function. Top: tilt magnitude
    ...
    ultimate-bound level $V_\infty=0.62$. After each disturbance, $V$ spikes and
...
of radius $\sim$$1.1^\circ$. Combining the three facts---(i) $V$ is bounded,
    (ii) the trajectory exhibits a bounded under-damped oscillation,
...
$80\,\text{s}$ run. The platform absorbs peak disturbances of $\sim$$3.1^\circ$
  and recovers consistently to the $\pm1^\circ$ band, with an average settling
  time of $5.06\,$s (a slow recovery that is a direct consequence of the missing
  damping term and the low PI gains chosen for vibration robustness).  The
  steady-state RMS tilt is $1.11^\circ$. Table~\ref{tab:summary} consolidates the
  real-time and control metrics. A live demonstration video showing the transient response and self-stabilizing performance of the physical prototype under external disturbances is available at \url{https://youtu.be/brQlPhEl86s}.
  ...
  schedulable at 500\,Hz ($1607 \le 2000\,\mu$s, 0 deadline misses over
$36{,}122$ cycles). Because the controller has no explicit damping term and no
  ...
  Lyapunov energy is bounded, while the $\dot V\le0$ fraction near $50\%$
