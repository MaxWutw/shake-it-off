...
\definecolor{downtitle_grey_white}{RGB}{240, 240, 240}
...
% Footer
% ==========================================
\setbeamercolor{author in head/foot}{bg=ntnured, fg=white}
\setbeamercolor{title in head/foot}{bg=downtitle_grey_white, fg=black!70}
\setbeamercolor{date in head/foot}{bg=ntnugrey, fg=black!70}
...
% Title info
% ==========================================
\title[Cyber-Physical Systems]{CSC9008 Cyber-Physical Systems \\ Shake it off: Checkpoint 2}
\author[Hon, Wu, Lin]{
...
\begin{frame}{Outline}
  \begin{enumerate}
    ...
    \item MPU6050: Reading and Parsing Sensor Data
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.8, every node/.style={font=\footnotesize}]
      ...
      \draw[fill=red!30, draw=red!70!black, thick]
        (-3.3, -1.7) rectangle (-2.7, -1.0);
      \draw[fill=red!30, draw=red!70!black, thick]
        (2.7, -1.7) rectangle (3.3, -1.0);
      \node[text=red!70!black] at (-3, -1.35) {\textbf{Servo}};
      \node[text=red!70!black] at (3, -1.35) {\textbf{Servo}};

      % Pulleys
      \draw[fill=yellow!50, draw=black] (-3, -0.6) circle (0.2);
      \draw[fill=yellow!50, draw=black] (3, -0.6) circle (0.2);

      % Cables (rotated platform endpoints)
      \draw[red!80!black, thick] (-2.45, 0.07) -- (-3, -0.6);
      \draw[red!80!black, thick] (2.55, 0.93) -- (3, -0.6);
      \draw[red!80!black, thick] (-3, -0.6) -- (-3, -1.0);
      \draw[red!80!black, thick] (3, -0.6) -- (3, -1.0);

      % IMU on platform
      \draw[fill=green!30, draw=green!70!black, thick, rotate around={10:(0, 0.5)}]
        (-1.5, 0.6) rectangle (-0.9, 0.8);
      \node[rotate=10, text=green!50!black] at (-1.2, 0.7) {\tiny IMU};
...
\begin{frame}{Signal Flow Diagram}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \draw[arr, dashed] (servo) -- node[left, font=\tiny]{physical} ++(-2.5, 0) |- (imu);
    \end{tikzpicture}
  \end{center}

  \begin{itemize}
    \item Closed-loop at \textbf{200 Hz} (5 ms control period)
...
\begin{frame}{Why Fuse Two Sensors?}
  \begin{center}
  \footnotesize
  \begin{tabular}{lcc}
    \toprule
...
\begin{frame}{Geometric Setup of the Mechanism}
  ...
  \begin{tikzpicture}[every node/.style={font=\Large}]
    % Base (orange)
    \draw[orange!80!black, line width=1.5pt] (-14, 0) -- (14, 0);
 
    % Servos
    \draw[fill=red!15, draw=red!70!black, line width=1.2pt]
      (-12.2, -1.0) rectangle (-10.6, 0);
    \draw[fill=red!15, draw=red!70!black, line width=1.2pt]
      (10.6, -1.0) rectangle (12.2, 0);
    \node[below=8pt, text=red!70!black] at (-11.4, -1.0) {$(-k,\,0)$};
    \node[below=8pt, text=red!70!black] at (11.4, -1.0) {$(k,\,0)$};
    \node[below=8pt, text=red!70!black] at (0, -1.0) {$(0,\,0)$};
 
    % Lower pillar (b)
    \draw[cyan!70!black, line width=4pt] (0, 0) -- (0, 4.5);
    \node[text=cyan!50!black, anchor=west] at (0.55, 2.25) {$b = 4.5$};
 
    % Universal joint
    \draw[fill=cyan!40, draw=black, line width=1.5pt] (0, 4.5) circle (0.45);
    \node[anchor=west, text=black!70] at (0.6, 3.85) {Joint $(0,\,b)$};
 
    % Upper pillar (a) -- label on right side to avoid overlap with a*cos, a*sin
    \draw[black, line width=4pt] (0, 4.5) -- (-0.68, 6.59);
    \node[anchor=west, text=black!80] at (0.05, 5.7) {$a = 2.2$};
 
    % Platform
    \draw[purple!70!black, line width=3.5pt] (-11.52, 3.07) -- (10.16, 10.11);
    \draw[fill=purple!70!black] (-11.52, 3.07) circle (0.35);
    \draw[fill=purple!70!black] (10.16, 10.11) circle (0.35);
 
    \node[anchor=south, text=purple!60!black] at (10.16, 10.55)
      {$P_R = (x_R,\,y_R)$};
    \node[anchor=south, text=purple!60!black] at (-11.52, 3.5)
      {$P_L = (x_L,\,y_L)$};
 
    % Cables
    \draw[green!50!black, line width=3pt] (-11.52, 3.07) -- (-11.4, 0);
    \draw[green!50!black, line width=3pt] (10.16, 10.11) -- (11.4, 0);
    \node[text=green!50!black, anchor=east] at (-11.75, 1.5) {$\ell_L$};
    \node[text=green!50!black, anchor=west] at (11.0, 5) {$\ell_R$};
 
    % Theta arc -- pushed slightly more to the left and label outside
    \draw[->, line width=1.5pt, blue!80!black] (0, 6.0) arc (90:108:1.5);
    \node[text=blue!80!black] at (-0.4, 6.55) {$\theta$};
 
    % a*sin, a*cos dashed -- labels moved further out
    \draw[dashed, cyan!60!black, line width=1pt] (-0.68, 6.59) -- (-0.68, 4.5);
    \draw[dashed, cyan!60!black, line width=1pt] (-0.68, 4.5) -- (0, 4.5);
    \node[text=cyan!60!black, anchor=east] at (-1.2, 5.55) {$a\cos\theta$};
    \node[text=cyan!60!black, anchor=north] at (-0.34, 4.15) {$a\sin\theta$};
 
    % k*sin, k*cos
    \draw[dashed, purple!60, line width=1pt] (10.16, 10.11) -- (10.16, 6.59);
    \draw[dashed, purple!60, line width=1pt] (10.16, 6.59) -- (-0.68, 6.59);
    \node[text=purple!60, anchor=south] at (4.74, 6.7) {$k\cos\theta$};
    \node[text=purple!60, anchor=west] at (10.4, 8.35) {$k\sin\theta$};
 
    % k width
    \draw[<->, dotted, line width=1.5pt] (0, -2.4) -- (11.4, -2.4);
    \node[anchor=north] at (5.7, -2.4) {$k = 11.4$};
  \end{tikzpicture}%
  }
  \end{center}
  \vspace{0.1cm}
...
\begin{frame}{Control Problem Statement}
  ...
  $\Rightarrow$ \textbf{PID controller}, the workhorse of feedback control.
\end{frame}

\begin{frame}{The Three Terms of PID (Continuous Form)}
...
\begin{frame}{From Discrete Positional to Velocity Form}
  ...
  \begin{block}{Discrete Positional PID}
    \[
      u_k = K_p\, e_k + K_i \cdot \Delta t \sum_{i=0}^{k} e_i + K_d \cdot \frac{e_k - e_{k-1}}{\Delta t}
    \]
  \end{block}

  \textbf{Drawback:} Requires accumulating historical errors, leading to integral windup.
  
  \begin{block}{Discrete Velocity Form PID (Incremental)}
     Subtracting $u_{k-1}$ from $u_k$ yields the required \textit{change} $\Delta u_k$ (in our system is the angle change for the next step):
    \[
      \Delta u_k = u_k - u_{k-1} = K_P\, [e_k - e_{k-1}] + K_I\, e_k + K_D\, [e_k - 2e_{k-1} + e_{k-2}]
    \]
  \end{block}
  \scriptsize (Note: Constants $K_P, K_I, K_D$ here absorb the $\Delta t$ and reflect the transformed terms.)
\end{frame}
...
\begin{frame}[fragile]{Implementation in C: Velocity Form PI Control}
...
\begin{frame}{From PID Output to Servo Action}
  PID outputs corrections $u_p$, $u_r$ in \textbf{degrees of attitude correction}.\\
...
\begin{frame}{Full Control Pipeline}
  \begin{enumerate}
    ...
    \item Output PWM via TIM2 channels (50 Hz, 1.0--2.0 ms pulse width).
  \end{enumerate}
